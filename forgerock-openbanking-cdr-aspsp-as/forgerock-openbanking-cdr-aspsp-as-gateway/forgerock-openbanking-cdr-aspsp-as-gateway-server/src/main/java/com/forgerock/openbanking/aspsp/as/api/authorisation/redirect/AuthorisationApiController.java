...
import com.forgerock.openbanking.aspsp.as.service.AccountRequestService;
    ...
    public ResponseEntity getAuthorisation(
        ...
        try {
            ...
            if (idTokenClaims.containsKey(("openbanking_intent_id"))) {
                ...
                String jwtScopeString = requestParameterJwt.getJWTClaimsSet().getStringClaim("scope");
                List<String> jwtScopes = jwtScopeString == null ? Collections.emptyList() : Arrays.asList(jwtScopeString.split(" "));
                String intentId = accountRequestService.registerConsent(jwtScopes, clientId).getData().getConsentId();
                idTokenClaims.put("openbanking_intent_id", new Claim(true, intentId));

                claims.put(OIDCConstants.OIDCClaim.ID_TOKEN, idTokenClaims.entrySet().stream()
                        .collect(Collectors.toMap(
                                e -> e.getKey(),
                                e -> e.getValue().toJson())
                        ));

                builder.claim(OIDCConstants.OIDCClaim.CLAIMS, claims);

                requestParametersSerialised = cryptoApiClient.signClaims(requestParameterJwt.getJWTClaimsSet().getIssuer(), builder.build(), false);
            }
    ...
    private SignedJWT validateRequestParameter(String responseType, String clientId, String state, String nonce, String scopes, String redirectUri, String requestParametersSerialised) throws OBErrorException {
        ...
        try {
            ...
            try {
                ...
                log.debug("Request parameter not signed by the TPP, but perhaps it's coming back to us with the request parameter we resigned ourself.");
                cryptoApiClient.validateJws(requestParametersSerialised, clientId, asGatewayJwkUri);
            }
