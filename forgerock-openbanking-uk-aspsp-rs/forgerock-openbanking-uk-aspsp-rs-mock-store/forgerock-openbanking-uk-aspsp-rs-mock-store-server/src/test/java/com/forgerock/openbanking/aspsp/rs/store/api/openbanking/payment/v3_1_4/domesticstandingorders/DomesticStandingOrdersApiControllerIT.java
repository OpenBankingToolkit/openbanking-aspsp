...
public class DomesticStandingOrdersApiControllerIT {
    ...
    private static final String CONTEXT_PATH = "/open-banking/v3.1.4/pisp/domestic-standing-orders/";
    ...
    public void testCreateDomesticStandingOrderPaymentSubmission() throws UnirestException {
        ...
        // When
        HttpResponse<OBWriteDomesticStandingOrderResponse5> response = Unirest.post(RS_STORE_URL + port + CONTEXT_PATH)
                .header(OBHeaders.X_FAPI_FINANCIAL_ID, rsConfiguration.financialId)
                .header(OBHeaders.AUTHORIZATION, "token")
                ...
                .asObject(OBWriteDomesticStandingOrderResponse5.class);

        // Then
        assertThat(response.getStatus()).isEqualTo(201);
        OBWriteDomesticStandingOrderResponse5 consentResponse = response.getBody();
    ...
    public void testCreateDomesticStandingOrderPaymentSubmission_expectRefundAccount() throws UnirestException {
        ...
        // Then
        assertThat(response.getStatus()).isEqualTo(201);
        OBWriteDomesticStandingOrderResponse5 rBody = response.getBody();
        FRDomesticStandingOrderPaymentSubmission submission = submissionRepository.findById(response.getBody().getData().getDomesticStandingOrderId()).get();
    ...
    public void testGetDomesticStandingOrderPaymentSubmission() throws UnirestException {
        ...
        // When
        HttpResponse<OBWriteDomesticStandingOrderResponse5> response = Unirest.get(RS_STORE_URL + port + CONTEXT_PATH + submission.getId())
                .header(OBHeaders.X_FAPI_FINANCIAL_ID, rsConfiguration.financialId)
                .header(OBHeaders.AUTHORIZATION, "token")
                .asObject(OBWriteDomesticStandingOrderResponse5.class);

        // Then
        OBWriteDomesticStandingOrderResponse5Data bodyData = response.getBody().getData();
