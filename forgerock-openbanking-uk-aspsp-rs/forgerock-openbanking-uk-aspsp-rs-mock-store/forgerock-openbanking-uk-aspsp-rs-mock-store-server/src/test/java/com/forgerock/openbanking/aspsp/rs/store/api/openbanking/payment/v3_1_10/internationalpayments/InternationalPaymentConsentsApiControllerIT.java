...
import com.forgerock.openbanking.aspsp.rs.store.api.openbanking.payment.v3_1_10.internationalpayments.InternationalPaymentConsentsApiController;
...
/**
 * Integration test for {@link InternationalPaymentConsentsApiController}.
 */
...
public class InternationalPaymentConsentsApiControllerIT {
    ...
    private static final String CONTEXT_PATH = "/open-banking/v3.1.10/pisp/international-payment-consents/";
    ...
    public void testGetInternationalPaymentConsent() throws UnirestException {
        // Given
        springSecForTest.mockAuthCollector.mockAuthorities(OBRIRole.ROLE_PISP);
        FRInternationalConsent consent = saveFRConsent(FRReadRefundAccount.NO, ConsentStatusCode.CONSUMED);

        // When
        HttpResponse<OBWriteInternationalConsentResponse6> response = Unirest.get(RS_STORE_URL + port + CONTEXT_PATH + consent.getId())
                .header(OBHeaders.X_FAPI_FINANCIAL_ID, rsConfiguration.financialId)
                .header(OBHeaders.AUTHORIZATION, "token")
                .asObject(OBWriteInternationalConsentResponse6.class);
        ...
        assertThat(toFRWriteInternationalDataInitiation(response.getBody().getData().getInitiation())).isEqualTo(consent.getInitiation());
    ...
    @Test
    public void testGetInternationalPaymentConsent_refundNull() throws UnirestException {
        // Given
        springSecForTest.mockAuthCollector.mockAuthorities(OBRIRole.ROLE_PISP);
        FRInternationalConsent consent = saveFRConsent(null, ConsentStatusCode.CONSUMED);
    ...
    @Test
    public void testGetInternationalPaymentConsentFunds() throws UnirestException {
        // Given
        springSecForTest.mockAuthCollector.mockAuthorities(OBRIRole.ROLE_PISP);
        FRInternationalConsent consent = saveFRConsent(FRReadRefundAccount.NO, ConsentStatusCode.AUTHORISED);
    ...
    public void testCreateInternationalPaymentConsent() throws UnirestException {
        ...
        // When
        HttpResponse<OBWriteInternationalConsentResponse6> response = Unirest.post(RS_STORE_URL + port + CONTEXT_PATH)
                .header(OBHeaders.X_FAPI_FINANCIAL_ID, rsConfiguration.financialId)
                .header(OBHeaders.AUTHORIZATION, "token")
                ...
                .body(aValidOBWriteInternationalConsent5())
                .asObject(OBWriteInternationalConsentResponse6.class);

        // Then
        log.error("The response: {}", response);
        assertThat(response.getStatus()).isEqualTo(201);
        OBWriteInternationalConsentResponse6 consentResponse = response.getBody();
        ...
        assertThat(toOBWriteInternational3DataInitiation(consent.getInitiation())).isEqualTo(consentResponse.getData().getInitiation());
    ...
    private FRInternationalConsent saveFRConsent(FRReadRefundAccount frReadRefundAccount, ConsentStatusCode consentStatusCode) {
        FRInternationalConsent consent = JMockData.mock(FRInternationalConsent.class);
        consent.getInternationalConsent().getData().setReadRefundAccount(frReadRefundAccount);
        consent.setId(IntentType.PAYMENT_INTERNATIONAL_CONSENT.generateIntentId());
    ...
    private static void setupTestFRConsentInitiation(FRWriteInternationalDataInitiation initiation) {
