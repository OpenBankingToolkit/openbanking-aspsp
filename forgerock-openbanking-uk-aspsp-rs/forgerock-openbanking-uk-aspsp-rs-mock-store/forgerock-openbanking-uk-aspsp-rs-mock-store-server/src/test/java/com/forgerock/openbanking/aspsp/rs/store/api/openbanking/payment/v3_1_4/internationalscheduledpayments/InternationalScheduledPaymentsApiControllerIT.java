...
public class InternationalScheduledPaymentsApiControllerIT {
    ...
    private static final String CONTEXT_PATH = "/open-banking/v3.1.4/pisp/international-scheduled-payments/";
    ...
    public void testCreateInternationalScheduledPaymentSubmission() throws UnirestException {
        ...
        // When
        HttpResponse<OBWriteInternationalScheduledResponse5> response = Unirest.post(RS_STORE_URL + port + CONTEXT_PATH)
                .header(OBHeaders.X_FAPI_FINANCIAL_ID, rsConfiguration.financialId)
                .header(OBHeaders.AUTHORIZATION, "token")
                ...
                .asObject(OBWriteInternationalScheduledResponse5.class);

        // Then
        assertThat(response.getStatus()).isEqualTo(201);
        OBWriteInternationalScheduledResponse5 consentResponse = response.getBody();
    ...
    public void testCreateInternationalScheduledPaymentSubmission_expectRefundAccount() throws UnirestException {
        ...
        // Then
        assertThat(response.getStatus()).isEqualTo(201);
        OBWriteInternationalScheduledResponse5 rBody = response.getBody();
        FRInternationalScheduledPaymentSubmission submission = submissionRepository.findById(response.getBody().getData().getInternationalScheduledPaymentId()).get();
    ...
    public void testGetInternationalScheduledPaymentSubmission() throws UnirestException {
        ...
        // When
        HttpResponse<OBWriteInternationalScheduledResponse5> response = Unirest.get(RS_STORE_URL + port + CONTEXT_PATH + submission.getId())
                .header(OBHeaders.X_FAPI_FINANCIAL_ID, rsConfiguration.financialId)
                .header(OBHeaders.AUTHORIZATION, "token")
                .asObject(OBWriteInternationalScheduledResponse5.class);

        // Then
        OBWriteInternationalScheduledResponse5Data bodyData = response.getBody().getData();
