 ...
 * under the License.
 */
package com.forgerock.openbanking.aspsp.rs.store.api.openbanking.account.v3_1_7.party;
...
/**
 * SpringBoot test for {@link com.forgerock.openbanking.aspsp.rs.store.api.openbanking.account.v3_1_7.party.PartyApiController}.
 */
...
public class PartyApiControllerIT {
    ...
    public void getAccountParties_returnTwo() {
        ...
        // When
        HttpResponse<OBReadParty3> response = Unirest.get("https://rs-store:" + port + "/open-banking/v3.1.7/aisp/accounts/"+accountParty.getAccountId()+"/parties")
    ...
    public void getAccountParty() {
       ...
       // springSecForTest.mockAuthCollector.mockAuthorities(OBRIRole.ROLE_AISP);
        ...
        // When
        HttpResponse<OBReadParty2> response = Unirest.get("https://rs-store:" + port + "/open-banking/v3.1.7/aisp/accounts/"+accountParty.getAccountId()+"/party")
                .header(OBHeaders.X_FAPI_FINANCIAL_ID, rsConfiguration.financialId)
                .header(OBHeaders.AUTHORIZATION, "token")
                .header("x-ob-url", "http://localhost")
                .header("x-ob-permissions", OBExternalPermissions1Code.READPARTY.name())
    ...
    public void getParty() {
        ...
        // When
        HttpResponse<OBReadParty2> response = Unirest.get("https://rs-store:" + port + "/open-banking/v3.1.7/aisp/party")
                .header(OBHeaders.X_FAPI_FINANCIAL_ID, rsConfiguration.financialId)
                .header(OBHeaders.AUTHORIZATION, "token")
                .header("x-ob-url", "http://localhost")
                .header("x-ob-permissions", OBExternalPermissions1Code.READPARTY.name())
