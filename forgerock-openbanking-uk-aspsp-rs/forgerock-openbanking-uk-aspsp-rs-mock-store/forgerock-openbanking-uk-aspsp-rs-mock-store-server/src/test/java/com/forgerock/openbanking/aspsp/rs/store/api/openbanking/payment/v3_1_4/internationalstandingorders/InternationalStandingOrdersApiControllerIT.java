...
import static com.forgerock.openbanking.common.services.openbanking.converter.payment.FRWriteInternationalStandingOrderConsentConverter.toOBWriteInternationalStandingOrder4;
...
public class InternationalStandingOrdersApiControllerIT {
    ...
    private static final String CONTEXT_PATH = "/open-banking/v3.1.4/pisp/international-standing-orders/";
    ...
    public void testCreateInternationalStandingOrderPaymentSubmission() throws UnirestException {
        ...
        // When
        HttpResponse<OBWriteInternationalStandingOrderResponse6> response = Unirest.post(RS_STORE_URL + port + CONTEXT_PATH)
                .header(OBHeaders.X_FAPI_FINANCIAL_ID, rsConfiguration.financialId)
                .header(OBHeaders.AUTHORIZATION, "token")
                ...
                .asObject(OBWriteInternationalStandingOrderResponse6.class);

        // Then
        assertThat(response.getStatus()).isEqualTo(201);
        OBWriteInternationalStandingOrderResponse6 consentResponse = response.getBody();
        ...
        assertThat(toOBWriteInternationalStandingOrder4(submission.getInternationalStandingOrder())).isEqualTo(submissionRequest);
        assertThat(submission.getVersion()).isEqualTo(OBVersion.v3_1_4);
    }
    ...
    public void testCreateInternationalStandingOrderPaymentSubmission_expectRefundAccount() throws UnirestException {
        ...
        // Then
        assertThat(response.getStatus()).isEqualTo(201);
        OBWriteInternationalStandingOrderResponse6 rBody = response.getBody();
        FRInternationalStandingOrderPaymentSubmission submission = submissionRepository.findById(response.getBody().getData().getInternationalStandingOrderId()).get();
        assertThat(submission.getId()).isEqualTo(rBody.getData().getConsentId());
        // Refund
        OBWriteInternationalResponse4DataRefund refund = rBody.getData().getRefund();
    ...
    public void testGetInternationalStandingOrderPaymentSubmission() throws UnirestException {
        ...
        // When
        HttpResponse<OBWriteInternationalStandingOrderResponse6> response = Unirest.get(RS_STORE_URL + port + CONTEXT_PATH + submission.getId())
                .header(OBHeaders.X_FAPI_FINANCIAL_ID, rsConfiguration.financialId)
                .header(OBHeaders.AUTHORIZATION, "token")
                .asObject(OBWriteInternationalStandingOrderResponse6.class);

        // Then
        OBWriteInternationalStandingOrderResponse6Data bodyData = response.getBody().getData();
