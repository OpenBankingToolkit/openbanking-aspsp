...
import com.forgerock.openbanking.common.model.openbanking.persistence.account.v1_1.FRStandingOrder1;
...
import uk.org.openbanking.datamodel.account.OBReadDataStandingOrder1;
...
public class StandingOrdersApiController implements StandingOrdersApi {
    ...
    public ResponseEntity<OBReadStandingOrder1> getAccountStandingOrders(
        ...
        Page<FRStandingOrder6> standingOrdersResponse = frStandingOrderRepository.byAccountIdWithPermissions(accountId, permissions,
                PageRequest.of(page, PAGE_LIMIT_STANDING_ORDERS));
        List<OBStandingOrder1> standingOrders = standingOrdersResponse.stream()
                .map(FRStandingOrderConverter::toStandingOrder1)
                .map(FRStandingOrder1::getStandingOrder)
                .map(so -> accountDataInternalIdFilter.apply(so))
                .collect(Collectors.toList());

        int totalPages = standingOrdersResponse.getTotalPages();

        return ResponseEntity.ok(new OBReadStandingOrder1()
                .data(new OBReadDataStandingOrder1().standingOrder(standingOrders))
                .links(PaginationUtil.generateLinks(httpUrl, page, totalPages))
                .meta(PaginationUtil.generateMetaData(totalPages)));
    }

    @Override
    public ResponseEntity<OBReadStandingOrder1> getStandingOrders(
