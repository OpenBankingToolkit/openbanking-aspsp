...
import com.forgerock.openbanking.commons.model.openbanking.v2_0.account.FRTransaction2;
...
public class FRTransaction2RepositoryImpl implements FRTransaction2RepositoryCustom {
    private static final Logger LOGGER = LoggerFactory.getLogger(FRTransaction2RepositoryImpl.class);

    @Autowired
    @Lazy
    private FRTransaction2Repository transactionRepository;

    @Override
    public Page<FRTransaction2> byAccountIdWithPermissions(String accountId, List<OBExternalPermissions1Code>
    ...
    @Override
    public Page<FRTransaction2> byAccountIdAndBookingDateTimeBetweenWithPermissions(String accountId, DateTime
    ...
    @Override
    public Page<FRTransaction2> byAccountIdAndStatementIdWithPermissions(
            String accountId,
            String statementId,
    ...
    @Override
    public Page<FRTransaction2> byAccountIdAndStatementIdAndBookingDateTimeBetweenWithPermissions(
            String accountId,
            String statementId,
    ...
    @Override
    public Page<FRTransaction2> byAccountIdInWithPermissions(List<String> accountIds, List<OBExternalPermissions1Code>
            permissions, Pageable pageable) {
    ...
    @Override
    public Page<FRTransaction2> byAccountIdInAndBookingDateTimeBetweenWithPermissions(List<String> accountIds,
    ...
    private Page<FRTransaction2> filterTransition(Page<FRTransaction2> transactions, List<OBExternalPermissions1Code> permissions) {
        for (FRTransaction2 transaction: transactions) {
            for (OBExternalPermissions1Code permission: permissions) {
                ...
                if (!permissions.contains(OBExternalPermissions1Code.READPAN)
                        ...
                        && transaction.getTransaction().getDebtorAccount().getSchemeName().equals(OBExternalAccountIdentification3Code.PAN)) {
                    transaction.getTransaction().getDebtorAccount().setIdentification("xxx");
                }
                if (!permissions.contains(OBExternalPermissions1Code.READPAN)
                        && transaction.getTransaction().getCreditorAccount() != null
                        && transaction.getTransaction().getCreditorAccount().getSchemeName().equals(OBExternalAccountIdentification3Code.PAN)) {
